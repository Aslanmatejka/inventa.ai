...
CATEGORY_VISUAL_KNOWLEDGE_EXPANDED: Dict[str, Dict[str, str]] = {
    ...
    "Engines & Powertrains": {
        "visual_profile": (
            "OVERALL FORM: Engines are dense, blocky assemblies with many external features. "
            "ICE engines: rectangular block with cylinder head on top, oil pan below, intake/exhaust on sides. "
            "Electric motors: cylindrical housing with shaft protruding from one end. "
            "Turbines: long cylindrical assembly with decreasing/increasing diameter sections. "
            "KEY VISUAL: Bolt bosses, cooling fins, gasket lines, pulleys, and fluid ports "
            "distinguish a realistic engine from a plain box. "
            "PROPORTIONS: Block height ≈ block width. Head is ~25% of block height. "
            "Oil pan is ~30% of block height below the block."
        ),
        "build_strategy": (
            "ORIENTATION: UPRIGHT — crankshaft axis is horizontal (Y or X), cylinders point up (Z). "
            "For ICE ENGINES: "
            "1. Engine block: main box at center, cut cylinder bores from top (N cylindrical holes). "
            "2. Cylinder head: box on top of block with bolt boss cylinders at corners. "
            "3. Valve cover: thinner box on top of head, slightly wider with lip. "
            "4. Intake manifold: plenum box on one side + runner tubes connecting to head. "
            "5. Exhaust manifold: shorter tubes on opposite side merging to collector. "
            "6. Oil pan: box below block, slightly narrower with drain plug boss. "
            "7. Front accessories: pulley cylinders on front face (crankshaft, alternator, water pump). "
            "8. Flywheel: flat cylinder on rear face. "
            "9. Mounting brackets: ear-shaped boxes on sides. "
            "10. Surface details: cooling fin cuts, head bolt circles, gasket lines. "
            "For ELECTRIC MOTORS: "
            "1. Main body: cylinder with circumferential cooling fin cuts. "
            "2. Shaft: smaller cylinder protruding from front. "
            "3. Mounting flange: wider disc on front face with bolt hole circle. "
            "4. Terminal box: small box on top or side with cable port. "
            "5. Rear cover: flat disc on back face."
        ),
        "recognition_features": (
            "1. CYLINDER BORES or STATOR: Internal cylindrical features defining engine type. "
            "2. EXTERNAL ACCESSORIES: Pulleys, manifolds, alternator — engines are never bare blocks. "
            "3. BOLT BOSSES: Visible bolt heads/bosses at all joints (head-to-block, cover, mounts). "
            "4. FLUID PORTS: Oil drain, coolant connections, fuel line fittings visible. "
            "5. MULTI-MATERIAL LOOK: Different sections suggest cast iron block vs. aluminum head."
        )
    },

    "Robots & Automation": {
        "visual_profile": (
            "OVERALL FORM: Articulated multi-joint structures with distinct body segments. "
            "Humanoid robots: torso + head + 2 arms + 2 legs, proportions ~7:1 height-to-head. "
            "Industrial arms: base + turntable + links + joints + end effector, reach 300-1500mm. "
            "Mobile robots: wheeled/tracked platform with sensor tower and manipulation capability. "
            "KEY VISUAL: Visible joints (spheres or cylinders) connecting rigid links. "
            "Panel lines suggesting access hatches. Sensor arrays on head/chassis. "
            "Cable routing visible along limbs. Motor housings at each joint."
        ),
        "build_strategy": (
            "ORIENTATION: STANDING UPRIGHT — total height is Z. "
            "For HUMANOID ROBOTS: "
            "1. Torso: loft between 3 cross-sections (chest widest, waist narrow, hips medium). "
            "2. Head: sphere or rounded box, cut visor/face plate groove on front. "
            "3. Neck: short cylinder between head and torso. "
            "4. Arms (build with helper function for left & right): "
            "   a. Shoulder joint: sphere at torso side "
            "   b. Upper arm: cylinder hanging from shoulder "
            "   c. Elbow joint: sphere at end of upper arm "
            "   d. Forearm: slightly narrower cylinder "
            "   e. Hand: box with finger groove cuts "
            "5. Legs (same helper function pattern): "
            "   a. Hip joint: sphere at torso bottom "
            "   b. Thigh: cylinder descending from hip "
            "   c. Knee joint: sphere "
            "   d. Shin: cylinder "
            "   e. Foot: wedge box at ground level "
            "6. Union all parts. Add panel line grooves, sensor holes, cable channels. "
            "For ROBOTIC ARMS: "
            "1. Heavy base cylinder at Z=0. "
            "2. Turntable cylinder on base (J1 rotation). "
            "3. Links as box/tube shapes connected by joint cylinders. "
            "4. Each joint has perpendicular motor housing cylinder. "
            "5. End with tool flange (flat cylinder with bolt holes). "
            "6. Cable conduit along external face of each link."
        ),
        "recognition_features": (
            "1. ARTICULATED JOINTS: Visible spherical or cylindrical joints between segments. "
            "2. PROPORTIONED SEGMENTS: Body parts should look mechanically plausible. "
            "3. MOTOR HOUSINGS: Cylindrical bulges at each powered joint. "
            "4. PANEL LINES: Surface grooves suggesting access panels and modularity. "
            "5. END EFFECTOR: Gripper, tool, or sensor at the working end."
        )
    },

    "Vehicles": {
        "visual_profile": (
            "OVERALL FORM: Vehicles have aerodynamic compound-curved bodies with clearly defined zones. "
            "Cars: lower body + greenhouse (cabin) + hood + trunk, 4 wheels. ~4500mm long. "
            "Motorcycles: frame + engine + tank + wheels + seat, ~2100mm long. "
            "Bicycles: diamond frame + 2 wheels + handlebars + seat, ~1700mm long. "
            "KEY VISUAL: Wheel wells, window openings, panel gaps (door lines), "
            "lighting recesses (headlights/taillights), grille pattern on front. "
            "PROPORTIONS: Wheels are ~14-18% of car length in diameter. "
            "Cabin height is ~35% of overall height. Overhang ratio matters."
        ),
        "build_strategy": (
            "ORIENTATION: ON ITS WHEELS — ground at Z=0, height is Z, length is X. "
            "For CARS: "
            "1. Chassis: main box at ground level (length × width × ground-to-beltline). "
            "2. Wheel wells: 4 cylindrical cuts at corners for wheel clearance. "
            "3. Wheels: cylinders seated in wheel wells. "
            "4. Cabin: loft from beltline rectangle to smaller roof rectangle. "
            "5. Hood: angled box in front of cabin (sloping down toward nose). "
            "6. Trunk: lower box behind cabin. "
            "7. Windows: rectangular cuts in cabin sides. "
            "8. Door panel lines: groove cuts on sides. "
            "9. Headlights/taillights: recessed cuts on front and rear faces. "
            "10. Grille: array of horizontal slot cuts on front. "
            "For MOTORCYCLES: "
            "1. Frame tubes: swept circles along line paths. "
            "2. Engine block: box shape between frame members. "
            "3. Fuel tank: lofted shape on top of frame. "
            "4. Wheels: thin cylinders at front and rear. "
            "5. Fork: two parallel cylinders from steering head to front axle. "
            "6. Swingarm: two arms from pivot to rear axle. "
            "7. Seat, exhaust, handlebars as additional features."
        ),
        "recognition_features": (
            "1. WHEELS: Properly sized and positioned at corners or axle points. "
            "2. CABIN/COCKPIT: Enclosed or open operator space with window openings. "
            "3. AERODYNAMIC SURFACES: Smooth transitions, no abrupt perpendicular faces. "
            "4. PANEL LINES: Door gaps, hood/trunk seams visible as groove cuts. "
            "5. LIGHTING: Headlight and taillight recesses clearly defined."
        )
    },

    "Prosthetics & Medical": {
        "visual_profile": (
            "OVERALL FORM: Prosthetics blend organic ergonomic shapes with mechanical precision. "
            "Legs: socket (custom-fit cone) + pylon (structural tube) + foot (spring shape). "
            "Arms/Hands: socket + forearm tube + wrist + articulated fingers. "
            "Exoskeletons: strut frame + actuated joints + cuffs/interfaces. "
            "KEY VISUAL: Smooth socket surfaces (patient contact), visible connector plates "
            "between sections, alignment adjustment hardware. Clean clinical aesthetic. "
            "PROPORTIONS: Socket matches limb cross-section. Pylon is ~25-35mm diameter. "
            "Overall length matches the replaced anatomy."
        ),
        "build_strategy": (
            "ORIENTATION: UPRIGHT — socket at top, foot at bottom, Z is height. "
            "For BELOW-KNEE PROSTHETIC: "
            "1. Socket: loft between ellipses (wider at top, narrowing). Shell for hollow. "
            "2. Socket brim: slightly thickened/rolled lip at top edge. "
            "3. Upper connector: flat cylinder under socket. "
            "4. Pylon: hollow cylinder (tube) — the structural member. "
            "5. Lower connector: flat cylinder above foot. "
            "6. Foot: lofted wedge (wider at heel, narrowing to toe area). "
            "   Slight split between heel and forefoot for flex simulation. "
            "For PROSTHETIC HAND: "
            "1. Palm: rounded box with curved dorsal surface. "
            "2. Each finger: 3 box segments connected by sphere joints (use helper function). "
            "3. Thumb: 2 segments, offset and rotated for opposition. "
            "4. Wrist: cylinder adapter with rotation mechanism bore. "
            "5. Cable/tendon channels: groove cuts along palmar finger surfaces."
        ),
        "recognition_features": (
            "1. ANATOMICAL PROPORTIONS: Parts match real body segment lengths. "
            "2. CONNECTOR INTERFACES: Visible adapter plates between components. "
            "3. SOCKET ERGONOMICS: Smooth inner surface, comfortable brim. "
            "4. STRUCTURAL PYLON: Clean tube connecting socket to terminal device. "
            "5. CLINICAL FINISH: Smooth surfaces, no unnecessary protrusions."
        )
    },

    "Mechanical Assemblies": {
        "visual_profile": (
            "OVERALL FORM: Mechanisms are collections of precisely positioned moving parts. "
            "Gearboxes: rectangular housing with shaft bores on opposing faces. "
            "Clock movements: two parallel plates with gear shafts between them. "
            "Presses: C-frame or 4-post frame with hydraulic cylinder and bed. "
            "Cranes: lattice tower + horizontal jib + counterweight. "
            "KEY VISUAL: Bolt holes at every joint. Shaft bores visible. "
            "Gear teeth around circumference. Frame members are structural sections."
        ),
        "build_strategy": (
            "ORIENTATION: UPRIGHT — main operating axis is Z. "
            "For GEARBOXES: "
            "1. Housing: box, shell out interior. "
            "2. Split line: cut groove around mid-height for upper/lower halves. "
            "3. Add bolt flange at split line with bolt holes. "
            "4. Cut shaft bores on opposing walls. "
            "5. Build gears as cylinders with circumferential tooth cuts. "
            "6. Add mounting feet below housing. "
            "For MECHANISMS (general): "
            "1. Build the FRAME first (structure that holds everything). "
            "2. Build each MOVING PART separately. "
            "3. Position parts at their HOME/REST position. "
            "4. Add BEARINGS/BUSHINGS at each rotation point. "
            "5. Add FASTENERS (bolt holes, mounting features). "
            "6. Add INDICATORS (scales, marks, limit stops). "
            "COMPLEXITY MANAGEMENT: "
            "- Use helper functions: def make_gear(r, teeth, thickness) "
            "- Build and validate each subsystem before final union "
            "- Name every part clearly: 'input_shaft', 'drive_gear', NOT 'part1'"
        ),
        "recognition_features": (
            "1. MOVING PARTS: Gears, levers, cams, sliders clearly identifiable. "
            "2. FRAME/HOUSING: Structural enclosure containing the mechanism. "
            "3. BEARING POINTS: Visible shaft holes or bearing seats. "
            "4. FASTENERS: Bolt holes and mounting features at all joints. "
            "5. FUNCTIONAL FORM: Every shape serves a mechanical purpose."
        )
    },

    "Aerospace": {
        "visual_profile": (
            "OVERALL FORM: Streamlined bodies optimized for aerodynamics. "
            "Aircraft: fuselage + wings + tail + engines + landing gear (1:50 scale for CAD). "
            "Rockets: cylindrical body + nose cone + fins + engine nozzles. "
            "Satellites: box body + solar panels + antennas. "
            "KEY VISUAL: Smooth fuselage curves (lofted between circular cross-sections). "
            "Wing airfoil shape (thin tapered cross-section). Control surfaces "
            "(ailerons, elevators, rudder) as hinged trailing-edge sections."
        ),
        "build_strategy": (
            "ORIENTATION: LEVEL FLIGHT — nose pointing +X, wings along Y, top is +Z. "
            "For AIRCRAFT: "
            "1. Fuselage: loft between 5-7 circular/elliptical cross-sections along X axis. "
            "   Nose section tapers to a point/spinner. Tail section tapers narrow. "
            "2. Wings: loft between airfoil-shaped cross-sections or use thin tapered box. "
            "   Attach at mid-fuselage, extend along ±Y. "
            "3. Horizontal stabilizer: smaller wing at tail. "
            "4. Vertical stabilizer: vertical fin rising from tail section. "
            "5. Engine: cylinder under wing or in nose (prop spinner or jet intake). "
            "6. Landing gear: cylinders with wheel discs, retractable into fuselage/wing. "
            "7. Cockpit: cut window openings in top of fuselage front section. "
            "For SATELLITES: "
            "1. Body: box (CubeSat standard: multiples of 100mm cubes). "
            "2. Solar panels: thin flat boxes angled from sides. "
            "3. Antennas: thin cylinders or discs. "
            "4. Sensors: small cylinders on various faces."
        ),
        "recognition_features": (
            "1. STREAMLINED BODY: Smooth lofted fuselage with no abrupt transitions. "
            "2. WING SHAPE: Thin tapered surfaces with proper aspect ratio. "
            "3. TAIL SURFACES: Horizontal + vertical stabilizers at rear of fuselage. "
            "4. PROPULSION: Engine/propeller/jet clearly visible. "
            "5. CONTROL SURFACES: Ailerons, elevators, rudder as distinct sections."
        )
    },
}
...
def format_visual_knowledge(product_name: str, category: str) -> str:
    ...
    # Universal arrangement guidance — always included
    lines.append("  📐 ARRANGEMENT RULES (CRITICAL for this product):")
    lines.append("    • ALL parts MUST physically touch/overlap — no floating gaps between components")
    lines.append("    • Derive EVERY position from parent dimensions (arm_tip_x = center_r + arm_len)")
    lines.append("    • Feature sizes proportional to body: buttons 5-12%, windows 15-30%, ports 12-18% of parent")
    lines.append("    • Symmetric products → symmetric features (even spacing, mirrored left/right)")
    lines.append("    • Arrays use even-spacing: pos = -span/2 + (i+1) * span/(count+1)")
    lines.append("    • Verify cutouts fall INSIDE body bounds: abs(position) < body_dim/2")
    lines.append("    • Z=0 is ground — everything chain-connected downward to ground plane")
    lines.append("    • Run mental connectivity check: component→parent→body→ground (no breaks)")
    lines.append("")
