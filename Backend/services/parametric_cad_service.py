...
class ParametricCADService:
    ...
    def _strip_centered_from_non_box(self, code: str) -> str:
        ...
        # Must handle multi-line .box() calls where centered= is on a different line.
        # First, find all spans of multi-line .box() calls so we can protect them.
        box_regions = set()  # line indices that are part of a .box() call
        lines = code.split('\n')
        in_box_call = False
        paren_depth = 0
        ...
        fixed_lines = []
        for idx, line in enumerate(lines):
            # Only strip centered= from lines NOT inside a .box() call
            if 'centered=' in line and idx not in box_regions:
    ...
    def _clamp_fillet_radii(self, code: str) -> str:
        ...
        if not dim_values:
            ...
            guard_val = round(smallest * 0.40, 2)
            guard_expr = str(guard_val)
            guard_comment = f"# Auto-guard: clamped to 40% of smallest dim ({smallest}mm)"
            ...
            if has_fillet:
                ...
                if not already_protected:
                    ...
                    if assignment_match:
                        ...
                        result_lines.append(f"{indent_str}except Exception as _fillet_err:")
                        result_lines.append(f"{indent_str}    print(f'⚠️ Fillet/chamfer skipped: {{_fillet_err}}')  # Auto-skip: radius too large for edge geometry")
                        i += 1
                        continue
        ...
        try:
            ...
            if hasattr(shape, 'Solids'):
                ...
                if len(solids) > 1:
                    ...
                    total_vol = sum(v for _, v in volumes)
                    largest_solid, largest_vol = max(volumes, key=lambda x: x[1])
                    # Only discard fragments — tiny slivers from boolean ops (< 1% of total)
                    fragments = [v for _, v in volumes if v < total_vol * 0.01]
                    if fragments and len(fragments) == len(solids) - 1:
                        # All other solids are tiny fragments — safe to keep only largest
                        print(f"  ⚠️ Multi-solid: {len(solids)} solids, {len(fragments)} fragment(s) removed (keeping largest)")
                        wp = cq.Workplane("XY")
    ...
    def _validate_output_quality(self, result: cq.Workplane, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if the result is a compound of separate (non-touching) solids
            try:
                solids = shape.Solids()
                solid_count = len(solids)
                quality["metrics"]["solid_count"] = solid_count
                if solid_count > 1:
                    # Check if solids are actually connected by testing bounding box overlaps
                    disconnected = 0
                    solid_bbs = [(s, s.BoundingBox()) for s in solids]
                    for idx, (s1, bb1) in enumerate(solid_bbs):
                        connected_to_any = False
                        for jdx, (s2, bb2) in enumerate(solid_bbs):
                            if idx == jdx:
                                continue
                            # Check bounding box overlap (with 1mm tolerance)
                            overlap_x = bb1.xmax + 1 >= bb2.xmin and bb2.xmax + 1 >= bb1.xmin
                            overlap_y = bb1.ymax + 1 >= bb2.ymin and bb2.ymax + 1 >= bb1.ymin
                            overlap_z = bb1.zmax + 1 >= bb2.zmin and bb2.zmax + 1 >= bb1.zmin
                    ...
                    quality["metrics"]["disconnected_solids"] = disconnected
                    if disconnected > 0:
                        quality["warnings"].append(f"⚠️ ASSEMBLY ERROR: {disconnected} disconnected/floating solid(s) detected — parts are not touching the main body")
                        quality["suggestions"].append("Ensure all parts overlap by at least 0.5mm when using .union(). Check translate positions.")
                        score -= 2.0
                    elif solid_count > 5:
                        quality["warnings"].append(f"📐 Model has {solid_count} separate solids — verify all parts are properly connected")
                        score -= 0.5
            except Exception:
                pass
            ...
            # ── Spatial arrangement quality checks ──
            
            # Check if model is grounded at Z≈0
            if bb.zmin < -1.0:
                quality["warnings"].append(f"⚠️ Model extends below ground: Z_min={bb.zmin:.1f}mm")
                score -= 0.5
            elif bb.zmin > 5.0:
                quality["warnings"].append(f"⚠️ Model floating above ground: Z_min={bb.zmin:.1f}mm (should be ≈0)")
                score -= 0.5
            
            # Check for extreme aspect ratios (pancake or needle shapes)
            sorted_dims = sorted(dims, reverse=True)
            if sorted_dims[0] > 0 and sorted_dims[2] > 0:
                aspect_ratio = sorted_dims[0] / sorted_dims[2]
                quality["metrics"]["aspect_ratio"] = round(aspect_ratio, 1)
                if aspect_ratio > 50:
                    quality["warnings"].append(f"⚠️ Extreme aspect ratio ({aspect_ratio:.0f}:1) — model may be paper-thin or needle-like")
                    score -= 1.0
                elif aspect_ratio > 20:
                    quality["warnings"].append(f"📐 High aspect ratio ({aspect_ratio:.0f}:1) — verify proportions are intentional")
                    score -= 0.3
            
            # Check for hardcoded positions (bad arrangement pattern)
            hardcoded_translates = 0
            for line in code.split('\n'):
                stripped = line.strip()
                if '.translate((' in stripped and not stripped.startswith('#'):
                    # Check if translate uses variables or raw numbers
                    import re as _re
                    translate_match = _re.search(r'\.translate\(\(([^)]+)\)', stripped)
                    if translate_match:
                        args = translate_match.group(1)
                        # Count numeric literals (not variable references)
                        nums = _re.findall(r'(?<![a-zA-Z_])(?:\d+\.?\d*|\.\d+)(?![a-zA-Z_])', args)
                        vars_found = _re.findall(r'[a-zA-Z_]\w*', args)
                        if len(nums) >= 3 and len(vars_found) == 0:
                            hardcoded_translates += 1
            
            if hardcoded_translates >= 3:
                quality["warnings"].append(f"⚠️ {hardcoded_translates} translates use hardcoded coordinates — may cause misalignment")
                quality["suggestions"].append("Use named variables (body_x/2, arm_length*cos(angle)) instead of magic numbers")
                score -= 0.5
    ...
    async def rebuild_with_parameters(
        ...
        if code_start == -1:
            raise ValueError("Cannot parse parametric script: missing GEOMETRY GENERATION marker")
        if code_end == -1:
            raise ValueError("Cannot parse parametric script: missing EXPORT marker")
        ...
        for k, v in updated_parameters.items():
            try:
                ...
                raise ValueError(f"Parameter '{k}' must be numeric, got {repr(v)}")
        namespace.update(coerced_params)
        
        # Safety validation (defense-in-depth — code was validated at initial build time too)
        self._validate_code_safety(code)
