...
function DraggableModel({
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      undefined,
      (error) => { if (!cancelled) console.error('STL loading error:', error); }
    );
...
function DimensionLine({ start, end, label, color, offset }) {
  ...
  const tickDir2 = offset ? new THREE.Vector3(...offset).normalize().multiplyScalar(tickLen) : new THREE.Vector3(0, tickLen, 0);
...
function MultiProductCanvas({ sceneId, initialProducts = [] }) {
  ...
  const handleTransformChange = useCallback((instanceId, transform) => {
    ...
    if (sceneId) {
      fetch(`${API_HOST}/api/scene/product/${instanceId}/transform`, {
        method: 'PUT',
  ...
  const handleDuplicate = useCallback(() => {
    ...
    fetch(`${API_HOST}/api/scene/product/${selectedId}/duplicate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteSelected = useCallback(() => {
    ...
    ids.forEach(id => {
      fetch(`${API_HOST}/api/scene/product/${id}`, { method: 'DELETE' })
  ...
  const handleAssemble = useCallback(() => {
    ...
    fetch(`${API_HOST}/api/scene/${sceneId}/assemble`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: 'Assembly',
  ...
  const handleDisassemble = useCallback(() => {
    ...
    fetch(`${API_HOST}/api/scene/assembly/${assembly.assemblyId}`, { method: 'DELETE' })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
