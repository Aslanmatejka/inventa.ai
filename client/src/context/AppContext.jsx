...
const initialState = {
  ...
  buildStartTime: null,
  streamBuildId: null,
  // Continuous modification tracking
  modificationStep: 0,
  // AI model selection
  selectedModel: '',
...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_STREAM_BUILD_ID':
      return { ...state, streamBuildId: action.payload };

    case 'BUILD_CANCELLED':
      return {
        ...state,
        status: state.result ? 'success' : 'idle',
        buildProgress: 0,
        buildStartTime: null,
        streamBuildId: null,
      };
    ...
    case 'BUILD_COMPLETE': {
      ...
      return {
        ...
        status: 'success',
        result: res,
        // Preserve currentDesign when result is null (ask/plan modes)
        currentDesign: res?.design ?? state.currentDesign,
        currentBuildId: res?.buildId || state.currentBuildId,
        currentProjectId: res?.projectId || state.currentProjectId,
        buildProgress: 100,
        streamBuildId: null,
        modificationStep: res?.design ? state.modificationStep + 1 : state.modificationStep,
        ...hist,
      };
    }
